...
MODEL_TYPE = "yolo"
REPO_ID = "HyunchanAn/Dental_009"
PT_PATH = "weights/best.pt" # Update path if different

def main():
    if not os.path.exists(PT_PATH):
        print(f"[Dental_009] No .pt file found at {PT_PATH}.")
        return

    print(f"[Dental_009] Exporting {PT_PATH} to ONNX...")
    ...
    if onnx_path and os.path.exists(onnx_path):
        print(f"[Dental_009] Uploading to HuggingFace {REPO_ID}...")
        api = HfApi()
        ...
        try:
            ...
            print(f"[Dental_009] Upload successful. Deleting local weights to save space...")
            os.remove(PT_PATH)
            os.remove(onnx_path)
            print(f"[Dental_009] Cleaned up local disk.")
        except Exception as e:
            print(f"[Dental_009] Upload failed: {e}")
